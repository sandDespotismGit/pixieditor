...
export default class DigitalClockWidget extends DraggableWidget {
  constructor(bounds, width, height, options = {}) {
    ...
    const isXLWithSeconds =
      width === 508 && height === 246 && options.showSeconds;
    ...
    const mainFontSize =
      options.fontSizeMain ?? (isLarge || isSmall ? 70 : 140);
    ...
    // Стиль для секунд
    const secondsFontSize = options.fontSizeAdd ?? (isLarge ? 70 : 70);
    ...
    this.contentContainer.mainTexts = [];
    this.contentContainer.separators = [];
    this.contentContainer.secondsText = null;
    ...
    if (isLarge) {
      ...
      this.contentContainer.mainTexts = [hourText, minutesText];
      this.contentContainer.separators = [colon1, colon2];
      this.contentContainer.secondsText = secondsText;
      ...
      const totalWidth =
        hourWidth + minutesWidth + secondsWidth + colonWidth * 2;
      ...
      this.contentContainer.addChild(hourText);
      xPos += hourWidth;
      ...
      this.contentContainer.addChild(colon1);
      xPos += colonWidth;
      ...
      this.contentContainer.addChild(minutesText);
      xPos += minutesWidth;
      ...
      this.contentContainer.addChild(colon2);
      xPos += colonWidth;
      ...
      this.contentContainer.addChild(secondsText);
    } else {
      // XL и S версии
      const hourMinutesText = new Text("00:00", styleMain);
      hourMinutesText.anchor.set(0.5);
      hourMinutesText.y = height / 2;
      this.contentContainer.addChild(hourMinutesText);
      this.contentContainer.mainTexts = [hourMinutesText];
    ...
    this.bg = bg;
    this.showSeconds =
      options.showSeconds !== undefined ? options.showSeconds : false;
  ...
  updateTime() {
    ...
    if (this.isLargeVersion) {
      ...
      this.contentContainer.mainTexts[0].text = hours; // часы
      this.contentContainer.mainTexts[1].text = minutes; // минуты

      if (this.contentContainer.secondsText) {
        this.contentContainer.secondsText.text = seconds;
      }
      ...
      this.contentContainer.mainTexts[0].text = `${hours}:${minutes}`;

      if (this.showSeconds && this.contentContainer.secondsText) {
        this.contentContainer.secondsText.text = seconds;

        // Обновляем позицию секунд для XL версии
        if (this._width === 508 && this._height === 246) {
          this.contentContainer.secondsText.x =
            this.contentContainer.mainTexts[0].x +
            this.contentContainer.mainTexts[0].width / 2 +
            10;
        }
      }
    }
  ...
  toggleSeconds(show) {
    ...
    if (this.contentContainer.secondsText) {
      this.contentContainer.secondsText.visible = this.showSeconds;
    }
