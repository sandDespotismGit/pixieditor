...
export default class CalendarWidget extends DraggableWidget {
  constructor(bounds, width, height, options = {}) {
    ...
    if (isCalendarXL) {
      createCalendarContent(this.contentContainer, width, height);
      this.type = "XL";
    } else if (isDateL) {
      createDateWithPreviousNextContent(
      ...
      createSimpleDateContent(this.contentContainer, width, height);
      this.type = "M";
    } else if (isDateM2) {
      createDateWithPreviousNextContent(
        this.contentContainer,
      ...
      createBigDateContent(this.contentContainer, width, height);
      this.type = "S";
    } else if (isDateXS) {
      if (options.dayOnly) {
        createDayOnlyContent(this.contentContainer, width, height);
        this.type = "XS_day";
      } else if (options.dateOnly) {
        createDateOnlyContent(this.contentContainer, width, height);
        this.type = "XS_date";
      } else {
        createDateWithDayContent(this.contentContainer, width, height);
        this.type = "XS";
      }
    }
  ...
  updateDate() {
    ...
    if (this.contentContainer.monthHeader) {
      this.updateCalendar(now);
    } else if (this.contentContainer.dateElements) {
      this.updateDateWithPreviousNext(now);
    } else if (this.contentContainer.dateElement) {
      this.updateSimpleDate(now);
    } else if (this.contentContainer.bigDateElements) {
      this.updateBigDate(now);
    } else if (this.contentContainer.dateWithDayElements) {
      this.updateDateWithDay(now);
    } else if (this.contentContainer.dayElement) {
  ...
  updateCalendar(date) {
    ...
    this.contentContainer.monthHeader.text = monthsFull[date.getMonth()];
    ...
    this.contentContainer.calendarCells.forEach((cell) => {
      cell.visible = false;
      cell.style = this.contentContainer.calendarStyles.styleDays;
    });

    this.contentContainer.calendarBackgrounds.forEach((bg) => {
    ...
    for (let row = 0; row < 6; row++) {
      for (let col = 0; col < 7; col++) {
        ...
        if (cellIndex >= firstDayOfWeek - 1 && day <= daysInMonth) {
          const cell = this.contentContainer.calendarCells[cellIndex];
          const cellBg = this.contentContainer.calendarBackgrounds[cellIndex]; // ← Получаем фон

          cell.text = day.toString();
          cell.visible = true;

          if (day === date.getDate()) {
            cell.style = this.contentContainer.calendarStyles.styleToday;
  ...
  updateDateWithPreviousNext(date) {
    ...
    for (let i = -2; i <= 2; i++) {
      ...
      if (this.contentContainer.dateElements[i + 2]) {
        this.contentContainer.dateElements[i + 2].text =
          `${dayText}, ${dateText} ${monthText}`;
      }
    }
  }
  ...
  updateSimpleDate(date) {
    ...
    this.contentContainer.dateElement.text = `${dayText}, ${dateText} ${monthText}`;
  }
  ...
  updateBigDate(date) {
    ...
    this.contentContainer.bigDateElements.day.text = date.getDate().toString();
    this.contentContainer.bigDateElements.month.text = months[date.getMonth()];
  }
  ...
  updateDateWithDay(date) {
    ...
    this.contentContainer.dateWithDayElements.date.text = `${date.getDate()} ${months[date.getMonth()]}`;
    this.contentContainer.dateWithDayElements.day.text = days[date.getDay()];
  }
  ...
  updateDayOnly(date) {
    ...
    this.contentContainer.dayElement.text = days[date.getDay()];
  }
