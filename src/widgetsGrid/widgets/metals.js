...
export default class MetalsWidget extends DraggableWidget {
  constructor(bounds, width, height, options = {}) {
    ...
    const isFullVersion = options.showAllMetals !== false;
    ...
    if (isFullVersion) {
      createMetalsContent(this.contentContainer, width, height);
      this.type = "metal-L";
    } else {
      createMetalsShortContent(this.contentContainer, width, height);
      this.type = "metal-S";
    }
    ...
    this._timer = setInterval(
      () => this.loadMetals(),
      options.updateInterval ?? 172800000,
    );
  }
    ...
    metals.forEach((metal) => {
      if (metalsData.data && metalsData.data[metal.index]) {
        ...
        if (metalResult.length >= 4) {
          ...
          const percChange = parseFloat(
            percentage.replace(",", ".").replace("%", ""),
          );
          const priceChange = (
            parseFloat(newPrice) * (1 + percChange / 100) -
            parseFloat(newPrice)
          ).toFixed(2);
          ...
          if (this.contentContainer[`${metal.prefix}DivideText`]) {
            this.contentContainer[`${metal.prefix}DivideText`].text =
              priceChange;
          }
          ...
          if (this.contentContainer[`${metal.prefix}Icon`]) {
            const iconName =
              percChange >= 0 ? "green_triangle.svg" : "red_triangle.svg";
            this.loadIcon(
              this.contentContainer[`${metal.prefix}Icon`],
              `${this.iconsBaseUrl}${iconName}`,
            );
          }
        }
      }
  ...
  showErrorState() {
    ...
    metals.forEach((metal) => {
      if (this.contentContainer[`${metal}PriceText`]) {
        this.contentContainer[`${metal}PriceText`].text = errorText;
      }

      if (this.contentContainer[`${metal}DivideText`]) {
        this.contentContainer[`${metal}DivideText`].text = errorText;
      }

      if (this.contentContainer[`${metal}PercText`]) {
        this.contentContainer[`${metal}PercText`].text = errorText;
      }

      // Устанавливаем красную иконку по умолчанию при ошибке
      if (this.contentContainer[`${metal}Icon`]) {
        this.loadIcon(
          this.contentContainer[`${metal}Icon`],
          `${this.iconsBaseUrl}red_triangle.svg`,
        );
      }
    });
  }
...
function createMetalsContent(container, width, height) {
  ...
  metals.forEach((metal, index) => {
    ...
    divideText.y = rowY;
    container.addChild(divideText);
    container[`${metal.key}DivideText`] = divideText;
    ...
    percText.y = rowY;
    container.addChild(percText);
    container[`${metal.key}PercText`] = percText;
    ...
    container[`${metal.key}PriceText`] = priceText;
    ...
    container[`${metal.key}Icon`] = trendIcon;
...
function createMetalsShortContent(container, width, height) {
  const styleName = new TextStyle({
    fontFamily: "Rubik",
    fontSize: 18,
    fill: 0xffffff,
