...
export default class WeatherWidget extends DraggableWidget {
  constructor(bounds, width, height, options = {}) {
    ...
    if (isWeatherXL) {
      createWeatherXLContent(this.contentContainer, width, height);
      this.type = "XL";
    } else if (isWeatherL) {
      createWeatherLContent(this.contentContainer, width, height);
      this.type = "L";
    } else if (isWeatherM) {
      createWeatherMContent(this.contentContainer, width, height);
      this.type = "M";
    } else if (isWeatherS) {
      createWeatherSContent(this.contentContainer, width, height);
      this.type = "S";
    }
  ...
  updateWeather(weatherData) {
    ...
    if (this.contentContainer.currentTempText) {
      this.contentContainer.currentTempText.text = currentWeather[0];
    }
    ...
    if (this.contentContainer.currentIcon && currentWeather[1]) {
      const iconUrl =
        this.weatherIcons[currentWeather[1]] || this.weatherIcons["Ясно"];
      this.loadIcon(this.contentContainer.currentIcon, iconUrl);
    }

    // Для XL виджета обновляем иконки по времени суток
    if (this.contentContainer.timeIcons && timeWeather.length >= 4) {
      const times = ["Утро", "День", "Вечер", "Ночь"];
      times.forEach((time, index) => {
        if (
          this.contentContainer.timeIcons[index] &&
          timeWeather[index] &&
          timeWeather[index][1]
        ) {
          const iconUrl =
            this.weatherIcons[timeWeather[index][1]] ||
            this.weatherIcons["Ясно"];
          this.loadIcon(this.contentContainer.timeIcons[index], iconUrl);
        }
      });
    }
  }
  ...
  showErrorState() {
    if (this.contentContainer.currentTempText) {
      this.contentContainer.currentTempText.text = "--°";
    }
    if (this.contentContainer.statusText) {
